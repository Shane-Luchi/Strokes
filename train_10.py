...
# 处理数据集
train_json_path = "data/train_data_10.json"
...
with open("data/data_vl_train_10.json", "w") as f:
    json.dump(train_data, f)
with open("data/data_vl_val_10.json", "w") as f:
    json.dump(val_data, f)
with open("data/data_vl_test_10.json", "w") as f:
    json.dump(test_data, f)

train_ds = Dataset.from_json("data/data_vl_train_10.json")
val_ds = Dataset.from_json("data/data_vl_val_10.json")
...
args = TrainingArguments(
    ...
    eval_strategy="epoch",  # 更新为 eval_strategy，避免弃用警告
...
# 修复 checkpoint 路径加载问题
checkpoint_path = "./output/Qwen2-VL-2B/checkpoint-100"  # 确保路径正确
val_peft_model = PeftModel.from_pretrained(model, checkpoint_path, config=val_config)

with open("data/data_vl_test_10.json", "r") as f:
    test_dataset = json.load(f)

test_image_list = []
for item in test_dataset:
    input_image_prompt = item["conversations"][0]["value"]
    true_output = item["conversations"][1]["value"]  # 获取真实输出用于对比
    ...
    # 打印预测和真实输出，便于调试
    print(f"Test Sample - True Output: {true_output}")
    print(f"Test Sample - Pred Output: {response}")